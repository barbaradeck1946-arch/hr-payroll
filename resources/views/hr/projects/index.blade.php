...
<div class="wrapper-page">
    <div class="page-title d-flex justify-content-between align-items-center">
        <h1><i class="icon-briefcase"></i> Projects</h1>
        @if(auth()->user()?->hasPermission('project.create'))
            <a href="{{ route('projects.create') }}" class="btn btn-custom"><i class="icon-plus"></i> Add Project</a>
        @endif
    </div>
    ...
    <div class="page-content"><div class="container-fluid"><div class="card no-border"><div class="content_wrapper" style="padding:20px;">
        <form method="GET" class="row g-2 mb-3">
            ...
            <div class="col-md-2"><select name="status" class="form-control"><option value="">All Status</option>@foreach(['draft','active','on_hold','completed','cancelled'] as $status)<option value="{{ $status }}" {{ $filters['status']===$status?'selected':'' }}>{{ ucfirst(str_replace('_',' ',$status)) }}</option>@endforeach</select></div>
        ...
        <div class="table-responsive">
            <table class="table table-bordered align-middle">
                <thead><tr><th>Name</th><th>Code</th><th>Team</th><th>Manager</th><th>Status</th><th>Progress</th><th>Tasks</th><th>Members</th><th>Actions</th></tr></thead>
                <tbody>
                    @forelse($projects as $project)
                        ...
                        <tr>
                            ...
                            <td class="action-buttons">
                                <a href="{{ route('projects.show', $project) }}" title="View"><i class="icon-eye"></i></a>
