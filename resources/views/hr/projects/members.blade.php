...
<div class="wrapper-page">
    ...
    <div class="page-content"><div class="container-fluid"><div class="card no-border"><div class="content_wrapper" style="padding:20px;">
        <form method="POST" action="{{ route('projects.members.sync', $project) }}">
            @csrf
            <div id="project-members-wrapper" class="d-grid" style="gap:10px;">
            ...
            <div class="mt-3 d-flex gap-2">
                <button type="button" id="add-project-member" class="btn btn-custom-default"><i class="icon-plus"></i> Add Member</button>
...
(function () {
    ...
    addBtn.addEventListener('click', function () {
        ...
        row.innerHTML = `
            <div class="col-md-5"><label>Employee</label><select name="members[${i}][employee_id]" class="form-control js-example-basic-single" required>${employeeOptions()}</select></div>
            <div class="col-md-2"><label>Role</label><select name="members[${i}][project_role]" class="form-control" required><option value="manager">Manager</option><option value="lead">Lead</option><option value="member" selected>Member</option><option value="observer">Observer</option></select></div>
            <div class="col-md-2"><label>Billable</label><select name="members[${i}][is_billable]" class="form-control"><option value="1" selected>Yes</option><option value="0">No</option></select></div>
            <div class="col-md-2"><label>Hourly Rate</label><input type="number" min="0" step="0.01" name="members[${i}][hourly_rate]" class="form-control"></div>
            <div class="col-md-1"><button type="button" class="btn btn-custom-default btn-sm remove-project-member"><i class="icon-trash"></i></button></div>
        `;
