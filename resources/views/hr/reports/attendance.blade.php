...
<div class="wrapper-page">
    <div class="page-title d-flex justify-content-between align-items-center">
        <h1><i class="icon-clock"></i> Attendance Report</h1>
        <a href="{{ route('reports.attendance.export', request()->query()) }}" class="btn btn-custom-default"><i class="icon-cloud-download"></i> Export CSV</a>
    </div>
        ...
        <div class="container-fluid">
            <div class="card no-border mb-3">
                <div class="content_wrapper" style="padding:20px;">
                    <div class="row g-3">
                        <div class="col-md-2"><strong>Present:</strong> {{ $summary['present'] }}</div>
                        <div class="col-md-2"><strong>Late:</strong> {{ $summary['late'] }}</div>
                        <div class="col-md-2"><strong>Absent:</strong> {{ $summary['absent'] }}</div>
                        <div class="col-md-2"><strong>Leave:</strong> {{ $summary['leave'] }}</div>
                        <div class="col-md-4"><strong>Worked Hours:</strong> {{ number_format(((float) $summary['worked_minutes']) / 60, 2) }}</div>
                    </div>
                </div>
            </div>
            ...
            <div class="card no-border">
                <div class="content_wrapper" style="padding:20px;">
                    <form method="GET" class="row g-2 mb-3">
                        ...
                        <div class="col-md-2"><select name="status" class="form-control"><option value="">All Status</option>@foreach(['present','late','absent','leave'] as $status)<option value="{{ $status }}" {{ $filters['status']===$status?'selected':'' }}>{{ ucfirst($status) }}</option>@endforeach</select></div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered align-middle">
                            <thead><tr><th>Date</th><th>Employee Code</th><th>Employee Name</th><th>Department</th><th>Status</th><th>Check In</th><th>Check Out</th><th>Worked</th><th>Source</th></tr></thead>
                            <tbody>
                                @forelse($logs as $log)
                                    <tr>
