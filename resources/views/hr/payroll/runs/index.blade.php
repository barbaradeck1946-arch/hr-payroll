...
<div class="wrapper-page">
    <div class="page-title"><h1><i class="icon-calculator"></i> Payroll Runs</h1></div>
            ...
            <div class="card no-border">
                <div class="content_wrapper" style="padding:20px;">
                    ...
                    <form method="GET" class="row g-2 mb-3">
                        ...
                        <div class="col-md-8 d-flex gap-2"><button class="btn btn-custom" type="submit"><i class="icon-magnifier"></i> Filter</button><a href="{{ route('payroll.runs.index') }}" class="btn btn-custom-default"><i class="icon-refresh"></i> Reset</a></div>
                    </form>

                    <div class="table-responsive">
                        <table class="table table-bordered align-middle">
                            <thead><tr><th>Period</th><th>Frequency</th><th>Items</th><th>Gross</th><th>Deductions</th><th>Net</th><th>Status</th><th>Finalized By</th><th>Actions</th></tr></thead>
                            <tbody>
                                @forelse($runs as $run)
                                    <tr>
                                        ...
                                        <td>{{ ucfirst($run->pay_frequency) }}</td>
                                        ...
                                        <td><span class="badge bg-secondary">{{ ucfirst($run->status) }}</span></td>
                                        <td>{{ $run->processor?->name ?: '-' }}</td>
                                        <td class="action-buttons"><a href="{{ route('payroll.runs.show', $run) }}" title="View"><i class="icon-eye"></i></a></td>
                                    </tr>
                                @empty
                                    <tr><td colspan="9" class="text-center">No payroll runs found.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
