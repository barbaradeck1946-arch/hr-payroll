...
<div class="wrapper-page">
    <div class="page-title">
        <h1><i class="icon-plane"></i> {{ $mode === 'edit' ? 'Edit Holiday' : 'Add Holiday' }}</h1>
    </div>
            ...
            <div class="card no-border">
                <div class="content_wrapper content-padded">
                    <form method="POST" action="{{ $mode === 'edit' ? route('holidays.update', $holiday) : route('holidays.store') }}">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 form-group mb-3">
                                ...
                                <select name="is_optional" class="form-control" required>
                                    <option value="0" {{ $isOptional === 0 ? 'selected' : '' }}>No</option>
                                    <option value="1" {{ $isOptional === 1 ? 'selected' : '' }}>Yes</option>
                                </select>
                            </div>
                        ...
                        <a href="{{ route('holidays.index') }}" class="btn btn-custom-default"><i class="icon-arrow-left"></i> Back</a>
                    </form>
                </div>
            </div>
        </div>
